...
export default function Navbar() {
  ...
  const closeMobile = () => { setMobileOpen(false); setMobileGenreOpen(false); };
                ...
                <AnimatePresence>
                  {searchResults.length > 0 && searchOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -8 }}
                      className="absolute top-12 right-0 w-[340px] max-w-[90vw] overflow-hidden"
                      style={{
                        background: 'rgba(6,6,17,0.97)',
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 bottom-0 w-[280px] z-[70] flex flex-col overflow-y-auto"
              style={{
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 pb-3">
                <Link to={user ? '/catalog' : '/'} onClick={closeMobile} className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-white shadow-[0_0_12px_rgba(255,255,255,0.25)]">
                    <Play className="w-3.5 h-3.5 text-black ml-0.5" fill="currentColor" />
                  </div>
                  <span className="text-lg font-black text-white">StreamFlix</span>
                </Link>
                <button onClick={closeMobile} className="p-2 rounded-lg" style={{ color: 'rgba(240,240,255,0.5)' }}>
                  <X className="w-5 h-5" />
                </button>
              </div>

              {/* Divider */}
              <div className="mx-5 h-px" style={{ background: 'linear-gradient(90deg, rgba(0,212,170,0.2), transparent)' }} />
              ...
              {user && (
                <div className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-white">{user.username}</p>
                      <p className="text-[11px]" style={{ color: 'rgba(240,240,255,0.45)' }}>{user.email}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <nav className="flex-1 px-3 py-2 space-y-1">
                <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-widest" style={{ color: 'rgba(0,212,170,0.6)' }}>
                  Explorar
                </p>

                {/* Películas */}
                <Link to="/catalog" onClick={closeMobile}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all"
                  style={{ color: 'rgba(240,240,255,0.8)' }}
                >
                  <Film className="w-4 h-4" style={{ color: '#00D4AA' }} />
                  Películas
                </Link>
                ...
                <div>
                  <button
                    ...
                    style={{ color: 'rgba(240,240,255,0.8)' }}
                  >
                    <Grid3X3 className="w-4 h-4" style={{ color: '#00D4AA' }} />
                    <span className="flex-1 text-left">Géneros</span>
                    <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${mobileGenreOpen ? 'rotate-180' : ''}`} style={{ color: 'rgba(240,240,255,0.4)' }} />
                  </button>
                  ...
                  <AnimatePresence>
                    {mobileGenreOpen && (
                      ...
                      >
                        <div className="pl-11 pr-3 pb-2 grid grid-cols-2 gap-1">
                          {genres.map(g => (
                            <Link
                              key={g.slug}
                              to={`/catalog?genre=${g.slug}`}
                ...
                {/* Para Ti */}
                {user && (
                  <Link to="/for-you" onClick={closeMobile}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all"
                    style={{ color: 'rgba(240,240,255,0.8)' }}
                  >
                ...
                {/* Mi Lista */}
                {user && (
                  <Link to="/my-list" onClick={closeMobile}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all"
                    style={{ color: 'rgba(240,240,255,0.8)' }}
                  >
                    <Heart className="w-4 h-4" style={{ color: '#00D4AA' }} />
                    Mi Lista
                  </Link>
                )}
                ...
                {user && (
                  <>
                    ...
                    <Link to="/profile" onClick={closeMobile}
                      ...
                      style={{ color: 'rgba(240,240,255,0.8)' }}
                    >
                      <User className="w-4 h-4" style={{ color: 'rgba(240,240,255,0.5)' }} />
                      Mi Perfil
                    </Link>

                    <Link to="/subscription" onClick={closeMobile}
                      className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all"
                      style={{ color: 'rgba(240,240,255,0.8)' }}
                    >
                      <Settings className="w-4 h-4" style={{ color: 'rgba(240,240,255,0.5)' }} />
                      Mi Plan
                    </Link>
